...
class MLInferenceService:
    """
    Service for ML model inference using SageMaker endpoints
    """
    
    def __init__(self):
        self.sagemaker_runtime = boto3.client('sagemaker-runtime')
        self.urlbert_endpoint = None
        self.xgboost_endpoint = None
        self.initialize_endpoints()
    
    def initialize_endpoints(self):
        """Initialize SageMaker endpoint names from environment variables"""
        import os
        self.urlbert_endpoint = os.environ.get('URLBERT_ENDPOINT_NAME')
        self.xgboost_endpoint = os.environ.get('XGBOOST_ENDPOINT_NAME')
        
        if not self.urlbert_endpoint:
            logger.warning("URLBERT endpoint not configured")
        if not self.xgboost_endpoint:
            logger.warning("XGBoost endpoint not configured")
    
    async def get_urlbert_prediction(self, url: str) -> Dict[str, Any]:
        """
        ...
        """
        if not self.urlbert_endpoint:
            logger.error("URLBERT endpoint not configured")
            raise RuntimeError("URLBERT endpoint not available")
        ...
        try:
            ...
            response = self.sagemaker_runtime.invoke_endpoint(
                EndpointName=self.urlbert_endpoint,
            ...
            prediction = self._process_urlbert_response(result)
            ...
            raise RuntimeError(f"URLBERT prediction failed: {str(e)}")
        except Exception as e:
            log_error(logger, e, {'operation': 'urlbert_prediction', 'url': url[:50]})
            raise RuntimeError(f"URLBERT prediction error: {str(e)}")
    
    async def get_xgboost_prediction(self, features: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        """
        if not self.xgboost_endpoint:
            logger.error("XGBoost endpoint not configured")
            raise RuntimeError("XGBoost endpoint not available")
        
        try:
            logger.info("Requesting XGBoost prediction")
            
            # Prepare features for XGBoost
            feature_vector = self._prepare_xgboost_features(features)
            input_data = {
            ...
            start_time = time.time()
            response = self.sagemaker_runtime.invoke_endpoint(
                EndpointName=self.xgboost_endpoint,
                ContentType='application/json',
                Body=json.dumps(input_data)
            )
            ...
            # Extract prediction data
            prediction = self._process_xgboost_response(result)
            ...
            log_error(logger, e, {'operation': 'xgboost_prediction', 'features': str(features)})
            raise RuntimeError(f"XGBoost prediction error: {str(e)}")
    
    def _process_urlbert_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid URLBERT response format: {str(e)}")
    
    def _process_xgboost_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"Invalid XGBoost response format: {str(e)}")
    
    def _prepare_xgboost_features(self, features: Dict[str, Any]) -> List[float]:
        """
    ...
    def create_ml_features(self, url: str, domain_analysis: Dict[str, Any], 
        ...
        features = {
            ...
            'external_threat_score': external_data.get('threat_score', 0.0),
        }
        
        return features
    
    async def get_combined_ml_prediction(self, url: str, domain_analysis: Dict[str, Any], 
                                        error_analysis: Dict[str, Any], 
                                        external_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            features = self.create_ml_features(url, domain_analysis, error_analysis, external_data)
            
            # Get predictions from both models (in parallel if possible)
            urlbert_prediction = None
            xgboost_prediction = None
            
            # Try URLBERT prediction
            try:
                urlbert_prediction = await self.get_urlbert_prediction(url)
            except Exception as e:
                logger.warning(f"URLBERT prediction failed: {e}")
            
            # Try XGBoost prediction
            try:
                xgboost_prediction = await self.get_xgboost_prediction(features)
            except Exception as e:
                logger.warning(f"XGBoost prediction failed: {e}")
            
            # Combine predictions
            return self._combine_predictions(urlbert_prediction, xgboost_prediction)
            
        except Exception as e:
            log_error(logger, e, {'operation': 'combined_ml_prediction', 'url': url[:50]})
            raise RuntimeError(f"Combined ML prediction failed: {str(e)}")
    
    def _combine_predictions(self, urlbert_prediction: Optional[Dict[str, Any]], 
        ...
        if not urlbert_prediction and not xgboost_prediction:
            raise RuntimeError("No ML predictions available")
